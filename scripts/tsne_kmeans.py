...
def plot_tsne_kmeans(latents, n_clusters=10):
...
def show_cluster(dataloader, model, n_clusters=30, examples_per_cluster=10):
    model.eval()
    ...
    for i in range(5):  # Mostrar 5 clusters
        cluster_indices = (labels == i).nonzero()[0]
        selected = cluster_indices[:examples_per_cluster]
        images = imgs[selected]
